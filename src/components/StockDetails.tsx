  ...
  return (
    <View
      style={[
        ...
        {
          backgroundColor:
            _getNoOfBags(stockTotal) > 10 ? Color.Green : '#EF5350',
        },
      ]}>
          ...
          <View style={{flex: 1, alignItems: 'center'}}>
            ...
            {User?.user_type == UserType.Shop_Seller && (
              <Text style={styles.titleSm}>
                Number of Bags: {getNoOfBags(stockTotal)}
              </Text>
            )}
          </View>
        </View>
      ...
      <View style={[styles.fill, {padding: 15}]}>
        <Text style={[styles.titleSm, {fontWeight: 'bold'}]}>
          Stock Issued:
        </Text>
        <Text style={styles.titleSm}>
          {formatQty(stockOut)} ({getNoOfBags(stockOut)} Bag
          {getNoOfBags(stockOut) == 1 ? '' : 's'})
        </Text>
      </View>
      ...
      <View style={[styles.fill, {alignItems: 'flex-end', padding: 15}]}>
        ...
        <Text style={styles.titleSm}>
          {formatQty(stockIn)} ({getNoOfBags(stockIn)} Bag
          {getNoOfBags(stockIn) == 1 ? '' : 's'})
        </Text>
      </View>
    </View>
  );
