...
interface ShopInspectionHistoryProps { }

const ShopInspectionHistory = (props: ShopInspectionHistoryProps) => {
  ...
  const renderItem = ({ item }) => <Card style={styles.card} onPress={() => navigation.navigate(Routes.CourtNotice,
    {
      data: { ...item, id: item.inspection_shops[0]?.id },
      trackingId: '',//step1Response?.inspection?.id.toString() + data.id.toString(),
      shop: item.inspection_shops[0]?.shops[0],
      shopInspection: true,
      ...
      <Card.Content>
        ...
        <View style={styles.row}>
          <View style={styles.cell}>
            <Text style={styles.txtBold}>Shop Name:{' '}</Text>
            <Text style={styles.txtSmall}>{item.inspection_shops[0]?.shops[0]?.title}</Text>
          </View>

          <View style={styles.cell}>
            <Text style={styles.txtBold}>Owner Name:{' '}</Text>
            <Text style={styles.txtSmall}>{item.inspection_shops[0]?.shops[0]?.owner_name}</Text>
          </View>
        </View>

        <View style={styles.row}>
          <View style={styles.cell}>
            <Text style={styles.txtBold}>Owner Father Name</Text>
            <Text style={styles.txtSmall}>{item.inspection_shops[0]?.shops[0]?.owner_father_name}</Text>
          </View>
          <View style={styles.cell}>
            <Text style={styles.txtBold}>Fine:</Text>
            <Text style={styles.txtSmall}>{' ' + item.inspection_shops[0]?.amount}</Text>
        ...
        <View style={styles.row}>
          <Text style={styles.txtBold}>Shop Address:{' '}</Text>
          <Text style={styles.txtSmall}>{item.inspection_shops[0]?.shops[0]?.address}</Text>
        </View>
      </Card.Content>
    </Card.Content>
  </Card>
